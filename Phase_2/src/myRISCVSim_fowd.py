...
def fetch():
    ...
    stall[0][0]=False
    if(stall_left==0):
        instruction_word[0][0] = '0x' + MEM[pc] + MEM[pc + 1]+ MEM[pc + 2] + MEM[pc + 3]
        instruction_exe+=1
    ...
    if(stall_left==0):
        for i in range(3):
            if(rs1[1][0]==prev_resisters[i][2] and rs1[1][0]!=""):
                if(opcode[1][0]=="0000011" or opcode[1][0]=="0100011"):
                    stall_left=max(i+1,stall_left)
                elif(i==2):
                    if(opcode[1][1]=="0000011" or opcode[1][1]=="0100011"):
                        stall_left=max(i+1,stall_left) 
                    else: 
                        forwarding_stage=1
                elif(i==1):
                    if(opcode[2][1]=="0000011" or opcode[2][1]=="0100011"):
                        stall_left=max(i+1,stall_left) 
                    else: 
                        operand1[1][0]=ALUResult[3-i][1]
                else:
                    if(opcode[3][1]=="0000011" or opcode[3][1]=="0100011"):
                        stall_left=max(i+1,stall_left) 
                    else: 
                        operand1[1][0]=ALUResult[3-i][1]
        for i in range(3):
...
def execute():
    ...
    if(forwarding_stage==1):
        operand1[1][0]=ALUResult[2][0]
        forwarding_stage=0
    if(forwarding_stage==2):
        operand2[1][0]=ALUResult[2][0]
        forwarding_stage=0
...
def mem():  
    ...
    if(stall[2][1]):
        ...
        return
    if(hex(ALUResult[2][1])=="0x10002028"):
        Exit1 = 1
        Exit2 = 1
        Exit3 = 1
        Exit4 = 1
    ...
    if (opcode[2][1] == "0000011"): # I
        ...
        if(binToHexa(func3[2][1]) == "0x0"):
            ...
            Data_Memory[hex(ALUResult[2][1])] = value[3][0]
